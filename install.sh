...
for i in openvpn php bower node unzip wget sed; do
  which $i > /dev/null
  if [ "$?" -ne 0 ]; then
    echo "Miss $i"
    exit
...
iptables -t nat -A POSTROUTING -s 10.8.8.0/24 -o $primary_nic -j MASQUERADE
iptables -t nat -A POSTROUTING -s 10.8.8.2/24 -o $primary_nic -j MASQUERADE
